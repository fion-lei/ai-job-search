...
describe("ageToDate", () => {
  ...
  test("converts weeks (a unit only seen live on this repo, not the US sibling)", () => {
    expect(ageToDate("2w", now)).toBe("2026-09-01");
  });
  ...
  test("converts years/long-running months (24mo, observed live)", () => {
    expect(ageToDate("24mo", now)).toBe("2024-09-25");
  });
  ...
  // Regression: this repo (unlike the US sibling) emits a literal "Date
  // unknown" for some rows, observed live 2026-09-15. It must fall through
  // to null exactly like any other unrecognized format, never a guess.
  test("returns null (not a guess) for the literal 'Date unknown'", () => {
    expect(ageToDate("Date unknown", now)).toBeNull();
  });
});
