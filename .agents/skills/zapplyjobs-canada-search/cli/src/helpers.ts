// Data source: the zapplyjobs/Canada-Jobs-2027 GitHub repo's README.md,
// fetched raw (no auth, no robots.txt restriction on raw.githubusercontent.com).
// Same tooling/family as zapplyjobs-search (zapplyjobs/New-Grad-Jobs-2027,
// US-only) but Canada-scoped, with different categories and - importantly -
// no built-in freshness cap: this repo's own postings range from minutes old
// to 24mo+ old (the US repo stays under ~2 weeks), and a "Posted" cell can
// literally read "Date unknown". There is no query/location search endpoint:
// the README holds one plain Markdown table per category, inside a <details>
// block. "Search" means: fetch the whole README once, parse every row in
// every category table, then filter client-side.
//
...
// a live Workday-backed listing, 2026-09-15, on the sibling US repo - same
// zapply.jobs redirect infrastructure). `detail` fetches that resolved page;
// many ATS pages populate `<meta property="og:description">` server-side
// even when the visible body is client-rendered, so that is tried first
// before falling back to a generic strip-tags extraction. A stale Apply link
// redirects back into zapply.jobs itself rather than erroring - see
// isUnresolvedZapplyUrl() below.
...
const UA = "Mozilla/5.0 (compatible; zapplyjobs-canada-search-cli/1.0)"
...
/**
 * Convert the source's relative "Posted" age ("13m", "2h", "3d", "2w", "1mo",
 * "24mo") to YYYY-MM-DD. Unlike the sibling US repo, this dataset also emits
 * a literal "Date unknown" for some rows (observed live, 2026-09-15) - the
 * regex simply does not match that, so it falls through to `null` exactly
 * like any other unrecognized format, never a guess.
 */
