#!/usr/bin/env bun
// Self-contained CLI over the zapplyjobs/New-Grad-Jobs-2027 GitHub repo - a
// crowdsourced, ~10-minutely-updated list of new-grad US roles (Software
// Engineering, Data/AI & Research, Security Engineering, Hardware & Systems
// Engineering, Business & Operations, Other). No external CLI framework, so
...
// US roles only. Keep volume reasonable - this fetches the README on every call.
...
const HELP = `zapplyjobs-cli — search new-grad US roles from zapplyjobs/New-Grad-Jobs-2027
...
SEARCH FLAGS
  ...
  --category <slug>      Scope to one section: software-engineering, data-ai-research,
                          security-engineering, hardware-systems-engineering,
                          business-operations, other-jobs. Default: all sections.
  --jobage <days>        Posted within N days (derived from the source's relative
                          minutes/hours/days age column - approximate, not exact).
...
EXAMPLES
  bun run src/cli.ts search -q "software engineer" -l "San Francisco" --format table
  bun run src/cli.ts search --category data-ai-research --sponsor-only --format table
  bun run src/cli.ts search -q "data engineer" --jobage 7 --format table
  bun run src/cli.ts detail kla_cloud-platform-engineer-a1b2c3 --format plain

Public data (GitHub repo README, US roles only), no authentication. Fetches the
