...
function matches(job: JobRow, opts: SearchOpts): boolean {
  if (!opts.includeClosed && job.closed) return false
...
function renderTable(jobs: JobRow[]): string {
  ...
  const rows = jobs.map((j) => {
    ...
    const date = j.date || "—"
    return `${j.id.slice(0, 24).padEnd(24)} ${title} ${company} ${loc} ${date}`
  })
  const header =
    "ID".padEnd(24) + " " + "TITLE".padEnd(42) + " " + "COMPANY".padEnd(24) + " " + "LOCATION".padEnd(22) + " DATE"
